...
Its report lands only when the run finishes (and may arrive as a `task-notification`, see "launched is NOT done" above), so **before** dispatching a `#technical-harness` run print the watch command `tail -f <session_dir>/harness-progress.log` (real `<session_dir>` substituted) so the developer can follow along; after the orchestrator returns, run `/harness-diff` to show the net diff of the session branch it stopped on.
