...
**Relaying a pending question (PD-ASK round-trip).** The orchestrator can end a turn with a pending question — typically *"apply the database migration now?"*. Relay it to the user (a plain question or `AskUserQuestion` is fine), collect their answer, then resume the orchestrator with the user's **verbatim decision, stated as the user's own choice** (e.g. *"The user chose: apply the migration"*). A relayed answer is authoritative — the orchestrator must act on it and must NOT re-ask for a "direct" confirmation. If it loops re-asking, your relay was too vague: restate it as an explicit user decision, don't just forward a system notification.
