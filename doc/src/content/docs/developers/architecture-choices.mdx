...
- `src/components/ui`: [Shadcn UI](https://ui.shadcn.com/), providing beautiful, accessible, and customizable components powered by Tailwind CSS.

You can modify or replace the files in these directories. It's the recommended way to customize the application.

Shared CRM primitives that are not tied to a single entity, such as reusable list filter widgets, live in the `src/components/atomic-crm/filters` directory.
