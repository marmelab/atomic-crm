...
This fork no longer ships the one-shot `make supabase-remote-init` bootstrap.
Create the project on [supabase.com](https://supabase.com/) first, then link and
deploy it from the repo:

```sh
npx supabase login
npx supabase link --project-ref <project-ref>
make remote-deploy-supabase
```

Then configure your production env file locally with the remote URL and publishable key.
...
This command now only builds the production bundle and serves `dist` locally.
If your env points to a remote Supabase project, it lets you exercise the
production build against that remote backend without deploying it.
