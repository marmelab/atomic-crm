...
The script will ask you for the Supabase instance name, create the database, apply the migrations, and set up the secrets needed by the edge functions. Finally, it will create a `.env.production.local` file with your remote Supabase configuration.
...
Set up secrets for the Supabase instance (follow this [guide](https://supabase.com/docs/guides/api/api-keys#where-to-find-keys) to find your publishable and secret keys):

```sh
npx supabase secrets set SB_PUBLISHABLE_KEY=<publishable_key> SB_SECRET_KEY=<secret_key>
```
...
The migrations create database triggers that call `delete_note_attachments` when notes are deleted or when attachments are removed from an existing note.

Atomic CRM also needs one extra secret for note attachment cleanup:

```sh
npx supabase secrets set ATTACHMENTS_WEBHOOK_SECRET=atomic-crm-note-attachments-webhook-secret
```
