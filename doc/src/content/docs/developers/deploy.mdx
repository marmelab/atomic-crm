...
In this fork, the supported frontend production path is Vercel auto-deploy on
push to `main`. A manual GitHub Pages deploy path is no longer maintained.
...
In this fork it deploys backend changes only: Supabase migrations plus Edge Functions on the linked remote project. The frontend keeps deploying from GitHub to Vercel separately.
...
The manual deploy workflow in this fork is backend-oriented: it updates the linked Supabase project. Frontend deployment remains handled by Vercel on `main`.
