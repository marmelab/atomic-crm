...
## Enable SSO in Atomic CRM


## Setup Google Workplace
...
8. **Save the following JSON in a mapping.json file**
   ```json
   {
     "keys": {
       "email": {
...
9. **Register the SSO Provider**

   Run the following command where the `metadata.xml` file is the file you downloaded at _step 4_ and `mapping.json` the file you created at _step 8_:
   ```sh
...
10. **Enable Google Workspace SSO in Atomic CRM**
