...
It's time to deploy your customized version of Atomic CRM to production. In this fork, [Supabase.com](https://supabase.com) hosts the backend and Vercel hosts the frontend.

First, login or register to [Supabase.com](https://supabase.com).

Create the remote Supabase project first, then link and deploy it from the repo:
...
Finally, configure your production env file locally with the remote URL and publishable key.

The frontend of the CRM is a Single-Page App that can be deployed to any CDN. In this fork, the supported production path is Vercel auto-deploy on push to `main`.
