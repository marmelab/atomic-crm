...
The Tags section lists all the tags used to label contacts. For each tag, you can:

- **Edit** it to change its name or color. The new name and color are immediately visible on all the contacts that have this tag.
- **Delete** it. The tag is removed from all the contacts that had it, without changing anything else on these contacts. You can undo the deletion for a few seconds using the "Undo" button of the notification.

Unlike the other settings, changes to tags are applied immediately: you don't need to click the Save button.
