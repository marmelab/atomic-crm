...
import { supabase, z } from './z';
import { Query, TableSchema } from '@rocicorp/zero';
...
const materializeAndWait = (query: Query<TableSchema>) =>
    new Promise(resolve => {
        query.materialize().addListener((snap, resultType) => {
            console.log({ snap, resultType });
            if (resultType === 'complete') resolve(snap);
        });
    });

const getQuery = (resource: string) => {
    // @ts-expect-error
    const query = z.query[resource];
    if (!query) throw new Error(`Resource '${resource}' not found`);
    return query as Query<TableSchema>;
};
...
const baseDataProvider: DataProvider = {
    getList: async (resource, params) => {
        ...
        const result = (await materializeAndWait(query)) as any[];
    ...
    getMany: async (resource, params) => {
        ...
        // return { data: json };
        const query = getQuery(resource);
        const result = (await materializeAndWait(
            query.where('id', 'IN', params.ids)
        )) as any[];
        console.log('getMany', resource, { result });
        return {
            data: result,
            total: result.length,
        };
