  ...
  return {
    ...
    rules: {
      ...
      fiscalDeadlineReminders: {
        ...
        meaning:
          "Un cron job automatico (pg_cron + Edge Function fiscal_deadline_check) calcola promemoria fiscali stimati del regime forfettario su due corsie: stima fiscale per anno di competenza e calendario di pagamento per anno solare. Le scadenze dell'anno Y sono costruite dalla stima di Y-1 e dal piano acconti derivato da Y-2, non dal cash dell'anno Y. I promemoria automatici (task di tipo f24, inps, bollo, dichiarazione) vengono creati per le scadenze entro 30 giorni e notificati internamente (email + WhatsApp) entro 7 giorni. Se parte del cash tassabile non e' mappato a un profilo ATECO valido, il sistema continua a generare il calendario ma lo tratta come stima degradata con warning strutturato. L'eventuale stato locale 'segnato come pagato' nella dashboard resta solo promemoria UI e non ridefinisce la verita' fiscale canonica.",
      },
      quoteToServiceConversion: {
