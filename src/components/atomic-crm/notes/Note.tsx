...
export const Note = ({
  ...
  const [deleteNote] = useDelete(resource, undefined, {
    ...
    onSuccess: () => {
      notify("crm.notes.notifications.deleted", {
        type: "info",
        undoable: true,
    ...
    >
      <div className="flex items-center space-x-4 w-full">
        ...
        <div className="inline-flex h-full items-center text-sm text-muted-foreground">
          ...
          {translate("crm.notes.added_a_note", { _: "added a note" })}{" "}
        ...
        <span className={`${isHover ? "visible" : "invisible"}`}>
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent>
                <p>{translate("crm.notes.edit_note", { _: "Edit note" })}</p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent>
                <p>
                  {translate("crm.notes.delete_note", { _: "Delete note" })}
                </p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
      ...
      {isEditing ? (
        <Form onSubmit={handleNoteUpdate} record={note} className="mt-1">
          ...
          <div className="flex justify-end mt-2 space-x-4">
            ...
            >
              ...
              {translate("crm.notes.update_note", { _: "Update note" })}
            </Button>
          </div>
        </Form>
      ) : (
        <div className="pt-2 text-sm max-w-150">
          ...
          {isTruncated && (
            ...
            >
              {isExpanded
                ? translate("crm.notes.show_less", { _: "Show less" })
                : translate("crm.notes.read_more", { _: "Read more" })}
            </button>
          )}
