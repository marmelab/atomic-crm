...
// Backs the Applications page's "1:1 — The Living Example" / "Growing
// Yourself Up → September / November" grouping (Runtime + Visual
// Consistency slice, §5): real Offer/Cohort relationships, not a new
// Application table or a hand-maintained grouping list. An Application
// with no matching deal/offer (shouldn't happen, but data can always be
// mid-migration) is simply omitted rather than crashing the page.
export const useApplicationsGrouped = () => {
  ...
  if (isPending) {
    return {
      isPending: true,
      individualGroups: [] as IndividualOfferGroup[],
      groupOfferGroups: [] as GroupOfferGroup[],
    };
  }
  ...
  return { isPending: false, individualGroups, groupOfferGroups };
};
