...
test.describe("Module: Payments - Complete", () => {
  ...
  test("payment detail shows linked project and calculations", async ({ page }) => {
    await loginAsLocalAdmin(page);
  ...
  test("edit payment updates status correctly", async ({ page }) => {
    ...
    const inAttesaRow = page.locator("table tbody tr", { hasText: "In attesa" }).first();
  ...
  test("mark payment as received from deadline tracker", async ({ page }) => {
    ...
    const markButton = page.getByRole("button", { name: "Segna come incassato" }).first();
  ...
  test("payment refund type has negative impact on balance", async ({ page }) => {
    await loginAsLocalAdmin(page);
