...
test.describe("Module: Clients - Complete CRUD", () => {
  ...
  test("clients list loads with correct columns and filters", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByRole("columnheader", { name: "Nome / Ragione sociale" }),
    ).toBeVisible();
    await expect(
      page.getByRole("columnheader", { name: "Tipo" }),
    ).toBeVisible();
    await expect(
      page.getByRole("columnheader", { name: "Telefono" }),
    ).toBeVisible();
    await expect(
  ...
  test("create new client with required fields", async ({ page }) => {
    ...
    await page
      .getByLabel("Indirizzo operativo / recapito")
      .fill("Via Roma 100, Milano");
  ...
  test("add and remove tags from client", async ({ page }) => {
    ...
    const addTagButton = page
      .getByRole("button", { name: /Aggiungi tag|Tag/ })
      .first();
  ...
  test("filter clients by type works correctly", async ({ page }) => {
    ...
    await page
      .getByText("Tipo cliente")
      .locator("..")
      .getByText("Azienda locale")
      .first()
      .click();
