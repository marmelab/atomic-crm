...
test.describe("Module: Projects - Complete", () => {
  ...
  test("projects list loads with filters and columns", async ({ page }) => {
    ...
    await expect(page.getByRole("button", { name: /Filtra per cliente/ })).toBeVisible();
  });
  ...
  test("project show displays financial summary correctly", async ({ page }) => {
    await loginAsLocalAdmin(page);
  ...
  test("quick episode dialog works", async ({ page }) => {
    ...
    // Verifica dialog
    await expect(page.getByRole("heading", { name: /Puntata|Aggiungi puntata/ })).toBeVisible();
  ...
  test("quick payment dialog works", async ({ page }) => {
    ...
    // Verifica dialog
    await expect(page.getByRole("heading", { name: /Pagamento|Registra pagamento/ })).toBeVisible();
  ...
  test("project contacts section shows linked contacts", async ({ page }) => {
    ...
    await expect(page.getByRole("button", { name: "Collega esistente" })).toBeVisible();
  });
