...
test.describe("AI Semantic UI", () => {
  test("AI launcher shows categorized questions with visual grouping", async ({ page }) => {
    ...
    await expect(page.getByText("Dammi un riepilogo operativo rapido")).toBeVisible();
    await expect(page.getByText("Dove vedi attenzione immediata")).toBeVisible();
    
    // Verifica descrizioni helper
    await expect(page.getByText("Panoramica generale di tutto")).toBeVisible();
  });

  test("AI semantic categories have distinct background colors", async ({ page }) => {
    await loginAsLocalAdmin(page);
    ...
    const revenueSection = page.locator('div[class*="from-emerald-50"]').first();
  ...
  test("AI shows enhanced loading state with pulse animation", async ({ page }) => {
    await loginAsLocalAdmin(page);
    ...
    await expect(page.getByText("Sto analizzando il CRM...")).toBeVisible({ timeout: 5000 });
    await expect(page.getByText("Leggo clienti, progetti, pagamenti")).toBeVisible();
    
    // Verifica che ci sia un elemento con animazione pulse
    const animatedElement = page.locator(".animate-pulse, .animate-ping").first();
  ...
  test("AI questions have icons and visual hierarchy", async ({ page }) => {
    ...
    const categoryHeaders = page.getByText(/Panoramica|Urgenti|Entrate|Lavoro|Insights/);
    expect(await categoryHeaders.count()).toBeGreaterThanOrEqual(3);
    
    // Verifica che ci siano icone (svg) nelle domande
    const questionIcons = page.locator("button svg, [class*='icon'] svg").first();
