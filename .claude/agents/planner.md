...
You DO a light codebase discovery to identify probable files DEVELOPER will touch — saves search time downstream. Use Grep / Glob. Do NOT reach for `LSP`: it is not available to a harness agent, and checking costs a turn (see `.claude/rules/code-search.md`). Light discovery only — no deep reading.
