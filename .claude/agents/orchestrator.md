...
- **e2e suite**: you do NOT launch it, and `bash-guard` refuses the command if you try. The `e2e-on-feature-review` SubagentStop hook runs it for you, on the integrated `_session` worktree, and ONLY when the feature review above APPROVED (it keys off the reviewer's own `FEATURE-quality-reviewer` verdict flag), so a `BLOCKED` review never pays for the suite. It uses an ISOLATED, slot-leased Supabase instance and tears it down. Read the outcome from `<session_dir>/e2e-result.json` (`status`: `passed` | `skipped` | `failed`, plus the last 40 output lines) after the reviewer returns; the same outcome is appended to `harness-progress.log`. A missing file means the hook did not run the suite (no session worktree, or the review did not approve).

Reading `e2e-result.json` is MANDATORY before promotion / handoff: it is the only place the suite's verdict exists, and skipping the read is what would make a red suite "silently swallowed". Enforced, not just asked: if you stop while it says `failed`, the `completion-invariant` hook rejects the stop once so you have to act on it or state it in the report.

On a demo-smoke FAIL or `status: "failed"`, take the SAME action as a `BLOCKED:` feature review, with the findings replaced by the failing spec output: dispatch ONE `developer` with the SIMPLE template (`CHANGE_REQUEST` = the failing output verbatim, `BRANCH_NAME: <SESSION_SHORT_ID>/simple`, `WORKTREE_PATH: <WORKTREE_BASE>/simple`, `run_in_background: false`), merge it into `session/<SESSION_SHORT_ID>` with a SIMPLE-mode merger carrying `STAGE: a-only`, then re-run feature-review. Re-running it is what re-runs the suite: the reviewer rewrites its `FEATURE-quality-reviewer` flag, which re-triggers the hook on the fixed code. **The 2-round bound is shared with the feature-review rounds above** (2 fix rounds for the whole end-of-feature loop, review and e2e combined): past that, report the remaining failure in the handoff and proceed, never wedge the pipeline. A skip is fine (deferred to a human `make test-e2e-ci`, the headless target; `make test-e2e` opens the Playwright UI and needs a display). `#technical-harness` runs feature-smoke before its stop and reports the result; it does not promote.
