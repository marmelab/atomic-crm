...
When your dispatch prompt carries `PERSONA: technical` (the `#technical-harness` opt-in), the request comes from a real developer who owns the merge and the deploy. Five behaviours change versus a normal run — they apply to every flow (SIMPLE, COMPLEX, SETUP):
...
5. **In-repo worktrees (visible in the editor).** So the developer can follow the work in VS Code's Source Control panel while it happens, this run's git worktrees live INSIDE the repo — `$CLAUDE_PROJECT_DIR/.harness-worktrees/<SESSION_ID>/` (a gitignored folder) — instead of `/tmp`. **Activate it by writing the marker once, at session start, before dispatching any developer:** `Bash("touch <session_dir>/.technical-persona")` (substitute the real `<session_dir>`). The marker is what makes `setup-worktree`/`cleanup-worktree` build and tear down the worktrees in-repo; the exact path formula you must substitute into `WORKTREE_PATH` lines is in the `WORKTREE_BASE` bullet under Environment. This is purely a location change — topology, branches, merges, and auto-cleanup are identical. `<session_dir>`, tickets, logs, and `harness-progress.log` stay in `/tmp` regardless.
...
- **WORKTREE_BASE** = `/tmp/<$CLAUDE_PROJECT_DIR with every "/" replaced by "_">/<SESSION_ID>`, where `<SESSION_ID>` is the full basename of `<session_dir>`. Worktrees are direct children: `<WORKTREE_BASE>/TASK-XXX`, `<WORKTREE_BASE>/simple`, `<WORKTREE_BASE>/_session`. Substitute the concrete path in dispatch prompts — never pass the literal `<WORKTREE_BASE>`. The repository itself stays at `$CLAUDE_PROJECT_DIR`.
  - **`PERSONA: technical` → `WORKTREE_BASE` = `$CLAUDE_PROJECT_DIR/.harness-worktrees/<SESSION_ID>`** instead (worktrees still direct children: `.../TASK-XXX`, `.../simple`, `.../_session`), so they land in a gitignored folder inside the repo and VS Code's Source Control panel shows each one live (behaviour 5 above). This only takes effect if you `touch <session_dir>/.technical-persona` at session start — the `setup-worktree`/`cleanup-worktree` hooks read that marker and create/remove the worktrees at this in-repo base. **Both must agree:** if the marker is set you MUST substitute the in-repo path into every `WORKTREE_PATH` line (the hook creates the worktree there, and a developer handed the `/tmp` path would `cd` into a directory that does not exist); if you cannot write the marker, use the `/tmp` base. `TICKETS_DIR`, logs, and `harness-progress.log` never move — only `WORKTREE_BASE` does.
