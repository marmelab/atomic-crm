...
// anywhere except the <base>/ops worktree. The developer writes migrations only
// when a dispatch sends it to the `writing-migrations` skill, which runs on the
// shared <base>/ops worktree; a per-ticket developer writes under <base>/TASK-XXX
...
// runs on the fixed <base>/ops worktree (the writing-migrations skill).
const isOpsWorktreeMigration = (p) => p.includes("/ops/supabase/migrations/");
...
const RULES = [
  {
    ...
    tag: "pending-folder",
    reason:
      "Writing to supabase/migrations-pending/ is not allowed. Migrations live in supabase/migrations/ and are written only via the writing-migrations skill (on the <base>/ops worktree).",
  },
  {
    blocked:
      baseAgent === "developer" &&
      isMigration(filePath) &&
      !isOpsWorktreeMigration(filePath),
    tag: "developer-migration",
    reason:
      "A per-ticket developer is not allowed to write SQL migrations. They are written only via the writing-migrations skill, on the <base>/ops worktree.",
  },
];
