...
### 3. Regenerate ONE net migration against what is ALREADY DEPLOYED (session-base)

"Deployed" = the migrations present at `session-base/<SESSION_SHORT_ID>` (the fork
anchor, i.e. the last deployed state). A migration YOU generated in an EARLIER
iteration of this same session is NOT deployed: it sits on the session branch but was
never pushed. Treating it as deployed is exactly what makes iterations STACK (add
column `foo`, then a second migration to rename it to `bar`, ...) instead of one clean
migration. The user does not want to push N migrations per change.

So, before authoring any SQL:

1. Find this session's own prior migration(s):
   `git diff --diff-filter=A session-base/<SESSION_SHORT_ID>..session/<SESSION_SHORT_ID> -- supabase/migrations/`
   (files ADDED since the fork = generated by an earlier iteration of this session).
2. **Delete them.** You will regenerate ONE fresh file that is the whole net change.
3. Author SQL for the NET delta between the desired schema (from the TS types) and the
   schema as of `session-base` ONLY. Never layer an incremental delta on top of your
   own prior migration.

Result: however many times the user iterates before deploy, the session carries
exactly ONE migration = `net(session-base -> final schema)`. If the net diff has no
schema impact, write **nothing** and report `NO_MIGRATION_NEEDED`.

**Hard boundary:** overwrite-and-regenerate is safe ONLY because the migration is not
yet deployed. Once it is pushed / promoted it is immutable; a later change is a NEW
forward migration, never an edit of the pushed one.

**Surface note.** Technical-harness writes to `supabase/migrations/`. On the web-chat
surface (CRM Builder) the launcher stages migrations in `supabase/migrations-pending/`
and promotes them to `supabase/migrations/` after the user confirms: follow the
launcher's staging, do not invent your own pending/promoted handling. The
regenerate-one-net-migration rule above applies either way (it is about NOT stacking,
independent of which directory the launcher stages into).

**Checkpoint:** at most ONE session-owned migration file exists, and its SQL is the
full net delta from `session-base` (not an increment on a prior session migration).
