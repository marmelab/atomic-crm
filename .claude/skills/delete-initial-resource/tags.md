...
The simplest of the five: `tags` is a **field** (`number[]`) on `Contact` plus a small standalone `tags` folder. No shared subsystem, no config prop, no aggregated column. (Read [`core.md`](core.md) first.)
...
- i18n: drop the `bulk_tag` block, `filters.tags`, and the `tags` resource block (both catalogs — see core.md).
